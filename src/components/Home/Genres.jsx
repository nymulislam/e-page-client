...
const genres = [
  { name: "Historical", icon: "🏛️", filter: "historical" },
  { name: "Mystery", icon: "🗝️", filter: "mystery" },
  { name: "Romance", icon: "❤️", filter: "romance" },
  { name: "Sci-Fi", icon: "🌌", filter: "sci-fi" },
  { name: "Fantasy", icon: "🧞‍♂️", filter: "fantasy" },
  { name: "Islamic", icon: "🌙", filter: "islamic" },
  { name: "Poetry", icon: "📜", filter: "poetry" },
  { name: "Horror", icon: "🕯️", filter: "horror" }
];
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto">
      <div className="text-center mb-12">
        ...
        <p className="text-amber-900/70">Find the perfect story for your mood.</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {genres.map((genre, index) => (
          <Link key={index} href={`/browse?category=${genre.filter}`}>
            <motion.div
            ...
            >
              {/* Islamic Pattern Accents (Corners) */}
